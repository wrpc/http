...
use wrpc_interface_http::{ServeIncomingHandlerWasmtime, ServeWasmtime};
...
struct Args {
    ...
    /// Path or URL to a component exporting `wasi:http/incoming-handler`
    workload: String,
}
...
impl<T: Send> ServeIncomingHandlerWasmtime<T> for Handler {
...
async fn main() -> anyhow::Result<()> {
    ...
    let wrpc = Arc::new(wrpc_transport::Server::default());
